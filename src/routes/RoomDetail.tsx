import { Avatar, AvatarBadge, Box, Container, Grid, GridItem, Heading, HStack, Image, Skeleton, Text, VStack } from "@chakra-ui/react";
    ...
    >
      ...
      <HStack width={"40%"} justifyContent={"space-between"} mt={10}>
        <VStack alignItems={"flex-start"}>
          ...
          <Skeleton isLoaded={!isLoading} h={"30px"}>
            <HStack justifyContent={"flex-start"} w="100%">
              <Text>{data?.toilets} toliet{data?.toilets === 1 ? "" : "s"}</Text>
              <Text>•</Text>
        ...
        <Avatar name={data?.owner.name} size={"lg"} src={data?.owner.avatar}>
          <AvatarBadge borderColor="white" boxSize="30px" bg="facebook.400" />
        </Avatar>
      </HStack>
      <HStack mt={10}>
        <Skeleton isLoaded={!isLoading} h={"30px"}>
        ...
        <Grid gap={10} templateColumns={"1fr 1fr"}>
          {reviewsData?.map((review, index) =>
            <VStack alignItems={"flex-start"} key={index}>
              ...
              <Skeleton isLoaded={!isLoading} w={"300px"} h={"30px"}>
